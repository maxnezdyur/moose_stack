...
Per-host env setup lives in:
- [`docs/local.md`](docs/local.md) — local machine (conda)
- [`docs/hpc.md`](docs/hpc.md) — INL HPC (container modules)

Paths below use `~/projects` as the parent. On INL HPC substitute `/scratch/$USER/projects`.
...
Create a sibling worktree of the meta-repo outside `moose_stack` so the baseline stays pristine. Put a feature branch on the meta-repo and on every submodule — branch names all match `<feature>` so later the meta-repo can bump its submodule pointers cleanly.
...
Then set up the env for this worktree — see [`docs/local.md`](docs/local.md) (conda) or [`docs/hpc.md`](docs/hpc.md) (HPC modules).
...
Remove submodule worktrees first, then the meta-repo worktree. Then tear down the env (see [`docs/local.md`](docs/local.md) or [`docs/hpc.md`](docs/hpc.md)). Delete feature branches only if nothing worth keeping lives on them.
