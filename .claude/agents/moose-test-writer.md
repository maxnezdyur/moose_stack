...
2. Find a sibling to mirror — `grep -rln "type = <Class>" <repo>/test/tests`; failing that, a test with the same Tester + physics shape. If grep doesn't surface a clear template, spawn `moose-scout` (below).
3. Author the input (tiny `GeneratedMesh`, small `num_steps`, minimal `[Outputs]`) and the spec (SQA fields; parent + `detail` for multi-test specs), per the standards.
...
When grep doesn't cleanly surface the closest template — which existing test most closely exercises this class/operator, which Tester + input shape tests of this kind use, or a parametrized test to extend — spawn `moose-scout` one-shot, read-only. Give it the class/operator, the scope, and what would NOT count as a match; use only its `file_path:line` cites. It surfaces facts; you author the test. If the spawn fails, report NEEDS_CONTEXT and the caller runs it.
