...
Your prompt carries one feature **spec slice** (compiled by `/moose-build` from `specs/spec.html`):
...
- `spec_path` — if a slice detail is missing, Grep it by contract-block id; don't whole-file Read (inlined KaTeX fonts bloat it)
