---
name: moose-spec
description: Takes a vague feature idea, grills the user against MOOSE-specific axes (object kind, inputs/outputs, physics/math), spawns `moose-scout` agents to scout for reusable code, and writes a structured, self-contained `specs/spec.html` that `/moose-build` consumes.
disable-model-invocation: true
---

# /moose-spec

Convert a vague feature idea into a concrete `specs/spec.html` for `/moose-build`: grill the user, scout the codebase for reuse, halt on near-matches, stop at a written spec. Does NOT auto-build — the human review pass on the spec is load-bearing.

## Usage

```
/moose-spec <freeform idea>
```

e.g. `/moose-spec postprocessor that integrates strain energy over a subdomain`. If the argument is empty, ask via `AskUserQuestion`.
...
If `<worktree-root>/specs/spec.html` already exists, `AskUserQuestion`: **Resume** (load it, grill only the sections whose contract blocks are empty or placeholder) / **Restart** (overwrite at step 6) / **Cancel**. A malformed spec on resume → restart with a warning.
...
- **Exact or near-exact match** → STOP the loop. Surface it (file:line + one-line description) and force a decision via `AskUserQuestion`:
  - **Reuse as-is** — no new code; spec captures only test/doc work
  - **Extend** — add a parameter, derived class, template specialization, virtual hook
  - **Write parallel** — user must give a one-sentence justification (recorded in the spec)
  - **Abandon idea** — feature already exists, no work needed
- **Close but not direct** → record it; next round asks "extend X or write fresh?"
- **No match** → record the negative result ("searched for X, Y, Z — nothing found") so the spec proves the search happened.
...
Scout findings are advisory — the user owns reuse decisions, recorded in the spec. If a scout returns BLOCKED or empty, continue without it that round and note "Scout failed: <reason>" under Reuse decisions; don't fabricate findings.
...
Then present a draft summary via `AskUserQuestion`: **Looks good — write it** / **Keep grilling about X** (user names the section) / **Cancel** (nothing written; tell the user "No spec saved. Re-run when ready.").

## 6. Write `<worktree-root>/specs/spec.html`

`mkdir -p <worktree-root>/specs`. The spec is the single deliverable — a self-contained HTML page that is both the human review artifact and the machine input to `/moose-build`.
...
Authoring — a pure formatter over the grill plan, scout findings, and user decisions; **never re-explores the codebase** (codegraph already ran via grill + scout; the spec skill's generic Explore/Design/Build workflows would bypass it):

1. Read the HTML skeleton from this skill's own [`references/plan-template.html`](references/plan-template.html) — a pinned copy of the global spec template; no cross-skill reads at authoring time. If it's missing, warn and author a plain self-contained HTML page instead — the seven contract blocks are the deliverable; the template is only the visual identity.
2. Fill it per [`references/spec-format.md`](references/spec-format.md) — the authoritative contract-block schema, template slot mapping, `.physics-pair` code↔math pairing, metadata header, and offline KaTeX rendering (`node <this skill's dir>/references/inline-katex.js <worktree-root>/specs/spec.html`; uses MOOSE's vendored KaTeX, degrades gracefully to plain-text LaTeX). The `#work-plan` block follows [`references/work-plan-format.md`](references/work-plan-format.md): implement units derived from the grill plan's predicted files (one per class), test units from `#test-plan`, doc unit from `#doc-plan`; edges only for hard dependencies; it **replaces** the template's per-phase task checklists.
3. Save to `<worktree-root>/specs/spec.html`. Self-check: all seven contract `id`s present and non-placeholder, no `{{` outside image-slot comments, no external `http(s)` stylesheet/script links, every `file:line` citation verbatim, plus the work-plan checks from `work-plan-format.md` (JSON parses, deps resolve + acyclic, same-file units share an edge, chips all `idle`, gate strips verbatim and absent from the JSON).

## 7. Stop

Tell the user:

> Spec written to `<worktree-root>/specs/spec.html` — open it in a browser to review. Edit if needed, then run:
>
> ```
> /moose-build specs/spec.html
> ```
