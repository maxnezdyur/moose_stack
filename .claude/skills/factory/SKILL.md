...
Relay these three parts and nothing else:
...
The waiting, parked and done groups stay on the board, as does any card the board did not put in
needs-you. Run nothing the board printed: `factory` writes the vault and nothing else, and the next
move is the user's to run. A non-zero exit is relayed verbatim as the first line: 1 means a
