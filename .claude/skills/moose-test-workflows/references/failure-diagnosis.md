...
- `Failed to import hit` → `$PYTHONPATH` interference or wrong env → `unset PYTHONPATH`, then activate the worktree's shared version-pinned env (`bash <meta-root>/scripts/moose-env.sh` prints it, e.g. `moose-8.19`)
